...
export function DashboardPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      {/* KPI Section - 4 Metric Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* ACTIVE PIPELINE */}
        <div className="bg-[#121214] border border-[#27272A] rounded-2xl p-5 transition-all hover:border-[#3F3F46]">
            ...
            <div className="space-y-4">
              {alerts.length > 0 ? (
                alerts.map((item) => (
                  <div
                    key={item.id}
                    className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-3 first:pt-0 border-t border-[#27272A] first:border-t-0"
                  >
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-center">
                      {item.actionRequired === 'Needs approval' ? (
                        <button
                          ...
                          className="px-4 py-1.5 rounded-full text-xs font-medium bg-[#1F1F23] text-zinc-200 hover:bg-blue-600 hover:text-white transition-all border border-[#2E2E33]"
                        >
                          Needs approval
                        </button>
                      ) : item.isEscalated ? (
                        ...
                        <button
                          ...
                          className="px-3 py-1 rounded-full text-xs font-medium bg-white/5 hover:bg-white/10 text-zinc-300 transition-colors"
                        >
                          Nudge
                        </button>
                      )}
